...
| **Bundled in repo** | `python/mt5_config/servers.dat` |
...
5. Copy the updated `servers.dat` from your Mac:
   ```bash
   cp ~/Library/Application\ Support/net.metaquotes.wine.metatrader5/drive_c/Program\ Files/MetaTrader\ 5/config/servers.dat python/mt5_config/servers.dat
   ```
6. Redeploy to VPS:
   ```bash
   scp python/mt5_config/servers.dat Administrator@<vps-ip>:'C:\MT5\mt5_config\servers.dat'
   ```
...
scp python/mt5_config/servers.dat Administrator@<vps-ip>:'%APPDATA%\MetaQuotes\Terminal\{hash}\config\servers.dat'
